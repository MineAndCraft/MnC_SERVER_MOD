...
public class PlacesManager
{
	...
	protected boolean isUserAllowedToTeleportTo(MinecraftPlayer player,Place port)
	{
		...
		try(PreparedStatement stat = DatabaseManager.getConnection().prepareStatement("SELECT 1 as permission FROM `mnc_places_permissions` pp WHERE pp.place_id = ? AND pp.user_id = ? LIMIT 1");)
		{
	...
	private Place getTeleport(String port)
	{
		...
		int owner_id = 0;
		int id = 0;
		try(PreparedStatement stat = DatabaseManager.getConnection().prepareStatement("SELECT num_id,name,welcome_message,x,y,z,world,type,owner_id FROM `mnc_places` WHERE `name` = ? LIMIT 1");)
		{ 
	...
	public boolean addTeleport(String portName,String portOwner,int x,int y, int z,String world, String type)
	{
		...
		try(PreparedStatement stat = DatabaseManager.getConnection().prepareStatement("INSERT INTO `mnc_places` (name,owner_id,x,y,z,type,world) SELECT ?,`id`,?,?,?,?,? FROM `mnc_users` WHERE username_clean=?");)
		{ 
	...
	{
		boolean success = false;
		try(PreparedStatement stat = DatabaseManager.getConnection().prepareStatement("DELETE `mnc_places`, `mnc_places_permissions` FROM `mnc_places` LEFT JOIN `mnc_places_permissions` ON `mnc_places`.`name`=`mnc_places_permissions`.`place_id` WHERE `mnc_places`.`name` = ?");)
		{
			stat.setString(1, portName.toLowerCase());
	...
	{
		try(PreparedStatement stat = DatabaseManager.getConnection().prepareStatement("INSERT IGNORE INTO `mnc_places_permissions` (user_id,place_id) (SELECT u.id,p.name FROM mnc_users u, mnc_places p WHERE p.name=? AND u.username_clean=? LIMIT 1)");)
		{
			stat.setString(1, portName.toLowerCase());
	...
	{
		boolean success = false;
		try(PreparedStatement stat = DatabaseManager.getConnection().prepareStatement("DELETE FROM `mnc_places_permissions` WHERE `user_id` = (SELECT `id` FROM `mnc_users` WHERE username_clean=?) AND `place_id`=?");)
		{
	...
	public ArrayList<Place> getPlacesByOwner(String owner)
	{
		...
		int owner_id = 0;
		int id = 0;
		try(PreparedStatement stat = DatabaseManager.getConnection().prepareStatement("SELECT num_id,name,welcome_message,x,y,z,world,type,owner_id FROM `mnc_places` WHERE owner_id = (SELECT `id` FROM mnc_users WHERE username_clean = ? LIMIT 1)");)
		{
	...
	public boolean isOwner(String portName, String name)
	{
		...
		int count=0;
		try(PreparedStatement stat = DatabaseManager.getConnection().prepareStatement("SELECT count(*) as count FROM `mnc_places` WHERE `owner_id` = ? AND `name`=?");)
		{
			stat.setInt(1,id);
	...
	{
		boolean success = false;
		try(PreparedStatement stat = DatabaseManager.getConnection().prepareStatement("UPDATE `mnc_places` SET welcome_message = ? WHERE `name`=? LIMIT 1");)
		{
			stat.setString(2, portName.toLowerCase());
	...
	public ArrayList<Place> getAccessiblePlaces(String playerName)
	{
		...
		try{
			stat = DatabaseManager.getConnection().prepareStatement("SELECT mnc_places.* FROM mnc_places WHERE mnc_places.type = 'public' OR `mnc_places`.`owner_id` = ? " +
...
" UNION SELECT mnc_places.* FROM mnc_places LEFT JOIN mnc_places_permissions ON mnc_places.name = mnc_places_permissions.place_id WHERE mnc_places.type = 'private' AND `mnc_places_permissions`.`user_id` = ?");
	...
	public ArrayList<Place> listAllPlaces()
	{
		...
		int owner_id = 0;
		int id = 0;
		try(PreparedStatement stat = DatabaseManager.getConnection().prepareStatement("SELECT num_id,name,welcome_message,x,y,z,world,type,owner_id FROM `mnc_places`;");)
		{
			ResultSet result = stat.executeQuery();
			while(result.next())
			{
	...
	public ArrayList<String> listTeleportAccess(String name)
	{
		...
		try(PreparedStatement stat = DatabaseManager.getConnection().prepareStatement("SELECT u.username as username FROM `mnc_places` p JOIN `mnc_places_permissions` pp ON p.name = pp.place_id JOIN `mnc_users` u ON pp.user_id=u.id WHERE p.name = ?");)
		{
	...
	public boolean modifyTeleport(String name, String newname, String owner, String type){
		...
		boolean success = false;
		try(PreparedStatement stat = DatabaseManager.getConnection().prepareStatement("UPDATE `mnc_places` SET `name` = ?, owner_id = ?, type = ? WHERE `name` = ? LIMIT 1;");)
		{
	...
	{
		boolean success = false;
		try(PreparedStatement stat = DatabaseManager.getConnection().prepareStatement("UPDATE `mnc_places` SET x = ?, y = ?, z = ?, world = ? WHERE name = ? LIMIT 1;");)
		{
	...
	public boolean modifyTeleportType(String name, String type) {
		...
		try(PreparedStatement stat = DatabaseManager.getConnection().prepareStatement("UPDATE `mnc_places` SET type = ? WHERE `name` = ? LIMIT 1;");)
		{	
